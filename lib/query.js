...
/**
 * Builds query search terms.
 * 
 * @param {string|Object} q - 
 * @param {enum} type - Allowed values in list ['AND', 'OR'].
 * @returns {string} URL encoded representation of Solr query.
 */
function buildQuery(q, type) {
  var result;
  if ( typeof(q) === 'string' ){
      result = encodeURIComponent(q);
   }else{
      result = querystring.stringify(q, '%20' + type + '%20',':');
   }
   return result;
}
...
Query.prototype.q = function(q){
   var self = this;
   var parameter ='q=' + buildQuery(q, 'AND');
   this.queryTerms = parameter;
   return self;
}

/**
 * Adds new query terms to the current query using 'AND' operator.
 *
 * @param {String|Object} q -
 *
 * @return  {Query}
...
Query.prototype.andQ = function(q){
   var self = this;
   if(this.queryTerms) {
     this.queryTerms += "%20AND%20" + buildQuery(q, 'AND');
   } else {
     this.q(q);
   }
   return self;
...
/**
 * Adds new query terms to the current query using 'OR' operator.
 *
 * @param {String|Object} q -
 *
 * @return  {Query}
...
Query.prototype.orQ = function(q){
   var self = this;
   if(this.queryTerms) {
     this.queryTerms += "%20OR%20" + buildQuery(q, 'OR');
   } else {
     this.q(q);
   }
   return self;
